#!/usr/bin/env python3
"""
Fixes a stale-closure bug in ImplementerMatches.tsx: the "no fresh matches"
branch checked React state (matches.length) through a closure captured
before the cache-paint setMatches() call updated it, so it always saw the
pre-paint empty array. This surfaced after the 45-point floor fix, because
an empty server response became a real possibility instead of near-never.
Fixed by tracking painted matches in a plain local variable instead.
"""
...
// 5, not 3, so it could show single-pair "instant AI fit" lookups (no
// score floor, triggered by directory browsing) as if they were real Top 3
// picks. Matches the 45-point floor and 3-item limit deployed server-side
// in refresh-partnership-matches v20.
//
// Fix 2: the "no fresh matches" branch used to check `matches.length > 0`
// (React state) to decide whether to keep showing the cache-painted result
// or fall back to empty. That read went through a stale closure captured
// before the cache-paint setMatches() call updated state, so it always saw
// the original empty array -- even when a match was correctly on screen.
// This rarely surfaced before, because the old unfiltered server response
// almost never came back genuinely empty. Fixed by tracking what got
// painted in a plain local variable (paintedMatches) instead of reading
// React state inside the closure.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cached && cached.length > 0 && !cancelled) {
        ...
        if (!cancelled) {
          paintedMatches = cached.map((m: any) => ({ ...m, org: orgMap.get(m.matched_org_id) }));

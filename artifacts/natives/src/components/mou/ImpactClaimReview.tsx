...
import { Loader2, CheckCircle2, AlertTriangle, Flag, X } from "lucide-react";
...
export default function ImpactClaimReview({
  ...
  return (
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      <div className="bg-white dark:bg-card rounded-t-2xl sm:rounded-2xl border border-border w-full sm:max-w-md shadow-xl p-6 space-y-4 max-h-[85vh] overflow-y-auto"
        ...
        <div className="rounded-lg border border-border p-3 space-y-1.5">
          ...
          <p className="text-sm text-black dark:text-white">{claim.claim_text}</p>

          <p className="text-sm font-medium text-black dark:text-white">Value: {claim.indicator_value}</p>

          <p className="text-xs text-black dark:text-white">

            Evidence: {claim.evidence_type === "link" ? (

              <a href={claim.evidence_value} target="_blank" rel="noopener noreferrer" className="underline">{claim.evidence_value}</a>
        ...
        {/* PENDING: counterparty can confirm or dispute (stays private -- never publicly confirmed) */}

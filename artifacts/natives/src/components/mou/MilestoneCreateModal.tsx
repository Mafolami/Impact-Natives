...
export default function MilestoneCreateModal({ mouDocumentId, orgA, orgB, myUserId, onClose, onCreated }: {
  ...
  async function createMilestone() {
    ...
    setCreating(true);
    const payerOrgId = payerSide === "org_a" ? orgA.id : payerSide === "org_b" ? orgB.id : null;
    const recipientOrgId = payerSide === "org_a" ? orgB.id : payerSide === "org_b" ? orgA.id : null;
      ...
      <div className="bg-white dark:bg-card rounded-t-2xl sm:rounded-2xl border border-border w-full sm:max-w-sm shadow-xl p-6 space-y-4"
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={createMilestone}
            disabled={creating || !title.trim() || (!!amount.trim() && payerSide === "none")}

...
// Redesigned Get Matched flow — sidebar step nav, decluttered layout,
// consistent single-select and multi-select patterns, full countries list.
//
// editMode={true}: skips Step 0 and pre-populates form from existing listing.
//   Used by PortfolioTable's Edit action on a partnership_listing row.
// editMode={false} (default): always starts at Step 0, even if the org has
//   a prior listing. Fixes the bug where returning users skipped describe.
...
const STEP_LABELS = [
  "Describe", "The partnership", "Where and when",
  "Focus areas", "Readiness", "Confirm",
];
...
function StepNav({ current, maxReached }: { current: number; maxReached: number }) {
  return (
    <div className="w-52 shrink-0 border-r border-border bg-muted/30 flex flex-col py-6 px-4 gap-1">
...
// Consistent step shell — header, scrollable body, sticky footer
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="shrink-0 px-8 pt-7 pb-5 border-b border-border">
        <p className="text-xs text-muted-foreground mb-1.5">Step {step + 1} of {STEP_LABELS.length}</p>
        <h2 className="text-xl font-bold text-foreground">{title}</h2>
        {subtitle && <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed max-w-xl">{subtitle}</p>}
      </div>
      ...
      <div className="shrink-0 px-8 py-5 border-t border-border bg-background flex items-center justify-between">
        {onBack ? (
        ...
        {onNext && (
          <button type="button" onClick={onNext} disabled={nextDisabled || loading}
            className="h-10 px-7 rounded-full text-sm font-semibold text-white bg-[#2D6A4F] hover:bg-[#245c43] disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-[0.98] flex items-center gap-2">
            {loading
...
// Field wrapper — label, optional hint, required marker
...
// Single-select radio cards — clean bordered rows, no scattered pills
          ...
          <button key={opt.value} type="button" onClick={() => onChange(opt.value === value ? "" : opt.value)}
            className={`w-full flex items-start gap-3 px-4 py-3.5 rounded-xl border text-left transition-all ${
              on
                ...
                : "border-border hover:border-[#2D6A4F]/40 hover:bg-muted/40"
            }`}>
            <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
              on ? "border-[#2D6A4F]" : "border-border"
            }`}>
              {on && <div className="w-2 h-2 rounded-full bg-[#2D6A4F]" />}
            </div>
            <div>
              <p className={`text-sm font-medium leading-snug ${on ? "text-[#2D6A4F]" : "text-foreground"}`}>
                {opt.label}
              </p>
...
// Multi-select checkboxes — clean rows instead of scattered pills
...
// Collapsible multi-select for long lists (sectors, countries, SDGs)
    ...
    <div className="border border-border rounded-xl overflow-hidden">
      <button type="button" onClick={() => setOpen(v => !v)}
        ...
        <span className="font-medium">
          {label}
          {selected.length > 0 && (
            <span className="ml-2 text-[11px] font-bold text-white bg-[#2D6A4F] rounded-full px-2 py-0.5">
              {selected.length}
            </span>
          )}
        </span>
        ...
        <div className="border-t border-border">
          <div className="px-3 py-2.5 border-b border-border">
            <input
              ...
              className="w-full h-8 px-3 rounded-lg border border-border bg-background text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-[#2D6A4F]/30"
            />
          </div>
              ...
              return (
                <label key={opt.value}
                  className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${
                    on ? "bg-[rgba(45,106,79,0.06)]" : "hover:bg-muted/30"
                  }`}>
                  ...
                  <span className={`text-sm ${on ? "font-medium text-[#2D6A4F]" : "text-foreground"}`}>
                    {opt.label}
                  </span>
                  <input type="checkbox" checked={on} onChange={() => onToggle(opt.value)} className="sr-only" />
                </label>
      ...
      {[{ v: true, label: yesLabel }, { v: false, label: noLabel }].map(opt => (
        <button key={String(opt.v)} type="button" onClick={() => onChange(opt.v)}
          className={`flex-1 py-3 rounded-xl border text-sm font-medium transition-all ${
            value === opt.v
              ? "border-[#2D6A4F] bg-[rgba(45,106,79,0.06)] text-[#2D6A4F]"
              : "border-border text-muted-foreground hover:border-[#2D6A4F]/40 hover:text-foreground"
          }`}>
          {opt.label}
        </button>
      ))}
...
function ConfirmRow({ checked, onChange, label, sub }: {
  checked: boolean; onChange: (v: boolean) => void; label: string; sub?: string;
}) {
  return (
    <label className={`flex items-start gap-4 px-4 py-4 rounded-xl border cursor-pointer transition-all ${
      checked ? "border-[#2D6A4F] bg-[rgba(45,106,79,0.06)]" : "border-border hover:border-[#2D6A4F]/30"
    }`}>
      <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
        checked ? "bg-[#2D6A4F] border-[#2D6A4F]" : "border-border"
      }`}>
        {checked && (
...
export function FindPartnerModalDashboard({
  ...
  const [appState, setAppState] = useState<
    "form" | "matching" | "results" | "no_org" | "rate_limited" | "new_request_prompt"
  >("form");
  ...
  useEffect(() => {
    ...
    setFormStep(0); setMaxReached(0); setAppState("form");
    setFreeText(""); setPartnershipTitle(""); setPrefillError("");
    ...
    async function loadOrg() {
      ...
      if (RATE_LIMIT_ENABLED) {
        ...
        const { data: recent } = await supabase.from("partnership_connections")
          .select("created_at").eq("sender_user_id", user!.id).gte("created_at", cutoff).limit(1);
        if (recent && recent.length > 0) { setAppState("rate_limited"); return; }
      }
      // Only skip Step 0 in editMode with existing data — not on fresh Get Matched runs
  ...
  async function submitAndMatch() {
    ...
    try {
      const { data: freshOrg } = await supabase.from("organizations")
  ...
  async function openComposer(match: MatchResult) {
    ...
    setDraftMessage(""); setDraftFailed(false); setDraftRequiresUpgrade(false);
  ...
  async function generateDraft(match: MatchResult) {
    ...
    try {
      ...
      const { data: receiverOrgRow } = await supabase.from("organizations")
        .select("user_id").eq("id", match.org.id).maybeSingle();
      let receiverContactName: string | null = null;
      if (receiverOrgRow?.user_id) {
        const { data: rp } = await supabase.from("profiles")
          .select("full_name").eq("id", receiverOrgRow.user_id).maybeSingle();
  ...
  async function sendInvite(match: MatchResult, message: string) {
    ...
    try {
      ...
      const { data: receiverProfile } = await supabase.from("organizations")
        .select("user_id").eq("id", match.org.id).single();
      const { data: convId, error: convError } = await supabase.rpc(
        "create_ai_match_conversation", {
      ...
      if (convId) {
        await supabase.from("messages").insert({
          conversation_id: convId, sender_id: user.id, body: message,
        });
      }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col"
      ...
      {/* ── Header ── */}
      <div className="shrink-0 flex items-center justify-between px-6 py-3.5 border-b border-border bg-background">
        <div className="flex items-center gap-2.5">
          ...
          {partnershipTitle && appState === "form" && formStep > 0 && (
            <span className="text-xs text-muted-foreground truncate max-w-[240px]">— {partnershipTitle}</span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-hidden flex min-h-0">
        ...
        {/* Utility states — no sidebar needed */}
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-8 text-center px-8">
            ...
            <div className="max-w-md">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                You've recently formed a partnership. Starting fresh replaces your current listing.
                Confirmed partners stay saved in Portfolio.
              </p>
            </div>
            <div className="flex gap-3 w-full max-w-xs">
              <button type="button" onClick={onClose}
                className="flex-1 h-11 rounded-full border border-border text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
                Cancel
              </button>
              <button type="button" onClick={async () => {
                if (!orgProfile) return;
                await supabase.from("organizations").update({
                  partnership_formed: false, partnership_listed: false,
                  partnership_title: null, partnership_sought: null,
                }).eq("id", orgProfile.id);
                const { data: freshOrg } = await supabase.from("organizations")
                  ...
                  .eq("id", orgProfile.id).single();
                setOrgProfile(freshOrg); setPartnershipTitle(""); setFreeText(""); setAppState("form"); goToStep(0);
              }} className="flex-1 h-11 rounded-full bg-[#2D6A4F] hover:bg-[#245c43] text-white text-sm font-semibold transition-colors">
                Start fresh
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-8 text-center px-8">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground max-w-sm leading-relaxed">
                Analysing needs, offers, sectors, SDG alignment, readiness signals, and working style...
              </p>
            </div>
          </div>
        )}
              ...
              {matches.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {(matchLimit === "all" ? matches : matches.slice(0, matchLimit)).map(match => {
                    ...
                    const countries = Array.isArray(match.org.country)
                      ? match.org.country
        ...
        {/* ── Form steps — sidebar + content ── */}
        {appState === "form" && (
          <>
            <StepNav current={formStep} maxReached={maxReached} />
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              {formStep === 0 && (
                <StepShell
                  ...
                  subtitle="Give your request a title and describe what you need — or upload a strategy document and let AI structure the brief."
                  onNext={runPrefill}
                  nextLabel="Structure with AI"
                  nextDisabled={(!freeText.trim() && !uploadedFile) || !partnershipTitle.trim()}
                  loading={prefilling}>

                  <Field label="Partnership title" required
                    hint="A short, specific title that describes this request.">
                    <input type="text"
                  ...
                  {uploadMode === "text" ? (
                    <Field label="Describe your partnership need"
                      hint="Include what you're working on, where, what kind of support you need, what you can offer, budget, and timeline. The more detail, the better the AI can structure your brief.">
                      <textarea rows={9}
                        ...
                        className="w-full px-4 py-3.5 rounded-xl border border-border bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]/20 focus:border-[#2D6A4F] transition-colors leading-relaxed" />
                    </Field>
                  ) : (
                    <Field label="Upload your partnership strategy document"
                      ...
                      {!uploadedFile ? (
                        <button type="button" onClick={() => fileRef.current?.click()}
                          className="w-full border-2 border-dashed border-border rounded-xl py-10 flex flex-col items-center gap-3 hover:border-[#2D6A4F]/40 hover:bg-[#2D6A4F]/5 transition-colors group">
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-xl border border-[#2D6A4F] bg-[rgba(45,106,79,0.08)]">
                      ...
                      {uploadedFile && (
                        <div className="mt-3">
                          ...
                          <textarea rows={3} placeholder="Any details not in the document..."
                            value={freeText} onChange={e => setFreeText(e.target.value)}
                            className="w-full px-4 py-3 rounded-xl border border-border bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]/20 transition-colors" />
                        </div>
                      )}
                    </Field>
                  )}
              ...
              {formStep === 1 && (
                <StepShell
                  ...
                  subtitle="What kind of partnership are you creating and what does it look like in practice."
                  onBack={() => goToStep(0)}
                  onNext={() => goToStep(2)}
                  ...
                  <Field label="What are you looking for?" required
                    ...
                    <Textarea
                      className="w-full text-sm resize-none rounded-xl border min-h-[110px]"
                  ...
                  <Field label="Organisation type" hint="Confirm or correct — AI may have misread this from your profile.">
                    <select
                      value={form.organisation_type}
                      onChange={e => setF("organisation_type", e.target.value)}
                      className="w-full h-11 px-4 rounded-xl border border-border bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]/20 focus:border-[#2D6A4F] transition-colors">
                  ...
                  <Field label="Expected duration">
                    <RadioCards
                      ...
                      onChange={v => toggleSingle("partnership_duration", v)}
                    />
                  </Field>
                  ...
                  <Field label="Partner exclusivity">
                    <RadioCards
                      options={[
                        ...
                        { value: "one_dedicated_partner",label: "One dedicated partner" },
                      ]}
                      value={form.partnership_exclusivity}
                      onChange={v => toggleSingle("partnership_exclusivity", v)}
                    />
                  </Field>
                </StepShell>
              )}
              ...
              {formStep === 2 && (
                <StepShell
                  ...
                  subtitle="Location, timeline, and resource signals save both sides from mismatched expectations before a single message is sent."
                  onBack={() => goToStep(1)}
                  onNext={() => goToStep(3)}
                  nextDisabled={form.country.length === 0}>

                  <Field label="Countries you operate in" required>
                    <ExpandableCheckList
                      label="Select countries"
                      options={COUNTRIES.map(c => ({ value: c, label: c }))}
                      selected={form.country}
                      onToggle={v => toggleArr("country", v)}
                    />
                  </Field>
                  ...
                  <Field label="Are you physically present in the target location?">
                    <YesNo
                      ...
                      yesLabel="Yes — on the ground"
                      noLabel="No — working remotely"
                    />
                  </Field>

                  <Divider label="Timeline and resources" />
                  ...
                  <Field label="Team capacity you can dedicate">
                    <RadioCards
                      ...
                      onChange={v => toggleSingle("partnership_team_capacity", v)}
                    />
                  </Field>
                </StepShell>
              )}
              ...
              {formStep === 3 && (
                <StepShell
                  ...
                  nextDisabled={
                    ...
                    !form.partnership_success_definition.trim()
                  }>

                  <Field label="Sectors" required hint="Select all that apply to this work.">
                    <ExpandableCheckList
                  ...
                  <Field label="What does success look like in 12 months?" required
                    ...
                    <Textarea
                      className="w-full text-sm resize-none rounded-xl border min-h-[80px]"
                  ...
                  <Divider label="Additional signals" />
              ...
              {formStep === 4 && (
                <StepShell
                  ...
                  subtitle="Showing what you have ready and being honest about your working style filters out mismatches before they waste your time."
                  onBack={() => goToStep(3)}
                  onNext={() => goToStep(5)}>

                  <Field label="Documents you have ready"
                    hint="Tick what you can share during due diligence. This is shown on your public listing.">
                    <CheckList
                      options={[
                  ...
                  <Field label="Who leads this partnership on your side?">
                    <RadioCards
                      ...
                      onChange={v => toggleSingle("partnership_contact_seniority", v)}
                    />
                  </Field>
                  ...
                  <Field label="Have you successfully completed a partnership before?">
                    <YesNo
                      value={form.partnership_prior_experience}
                      onChange={v => setF("partnership_prior_experience", v)}
                    />
                    {form.partnership_prior_experience === true && (
                      <div className="mt-4">
                        <Textarea
                          className="w-full text-sm resize-none rounded-xl border"
              ...
              {formStep === 5 && (
                <StepShell
                  step={5}
                  title="Final details"
                  subtitle="Your theory of change, any constraints partners need to know, and whether to list publicly."
                  onBack={() => goToStep(4)}
                  onNext={submitAndMatch}
                  nextLabel={listPublicly ? "List publicly and find matches" : "Find matches privately"}
                  loading={submitting}>

                  <Field label="Your approach to creating change"
                    hint="One sentence. Helps partners check if your theories of change are compatible — a common reason good-looking partnerships fail.">
                    <Textarea
                      className="w-full text-sm resize-none rounded-xl border"
                  ...
                  {!listPublicly && (
                    <p className="text-xs text-muted-foreground">
                      Your details won't be listed publicly. The Natives team will follow up with matches directly.
                    </p>
                  )}
                </StepShell>
              )}

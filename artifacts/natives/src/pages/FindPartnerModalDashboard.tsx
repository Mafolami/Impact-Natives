...
function StepFooter({onBack,onNext,nextLabel,nextDisabled,loading}:{
    ...
    <div className="border border-border rounded-lg overflow-hidden">
      <button type="button" onClick={()=>setOpen(v=>!v)}
        ...
        <span className="font-medium">
          {label}
          {selected.length>0 && (
            <span className="ml-2 text-[11px] font-bold text-white bg-[#2D6A4F] rounded-full px-2 py-0.5">{selected.length}</span>
          )}
        </span>
        {open?<ChevronUp className="w-4 h-4 text-muted-foreground"/>:<ChevronDown className="w-4 h-4 text-muted-foreground"/>}
      </button>
          ...
          <>
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              <StepFooter
                ...
                nextDisabled={!stepComplete(formStep,form,partnershipTitle,freeText,uploadedFile)}

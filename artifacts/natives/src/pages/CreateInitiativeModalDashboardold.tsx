...
type InitiativeStage =
  | "concept" | "planning" | "active" | "scaling"
...
const CONFIRMED_ASSET_OPTIONS: { value: string; label: string }[] = [
    ...
    <div className="flex gap-2">
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="h-[3px] flex-1 rounded-full transition-colors duration-300"
          style={{ background: i < current ? "#2D6A4F" : i === current ? "#C45C26" : "rgba(0,0,0,0.1)" }} />
      ))}
    </div>
  )
}
...
function ChipGroup<T extends string>({ options, selected, onToggle, getLabel, getDot }: {
  options: T[]; selected: T[]; onToggle: (v: T) => void;
  getLabel?: (v: T) => string; getDot?: (v: T) => string
}) {
  return (
      ...
      {options.map((v) => {
        ...
        return (
          <button key={v} type="button" onClick={() => onToggle(v)}
            ...
            {isSelected && getDot && (
              <span className="w-2 h-2 rounded-full shrink-0" style={{ background: getDot(v) }} />
            )}
...
function StepHeader({ step, title, total = 6 }: { step: number; title: string; total?: number }) {
  return (
    <div className="mb-7">
      <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-1">Step {step} of {total}</p>
      <h2 className="text-xl font-semibold text-foreground">{title}</h2>
    </div>
  )
}

function StepFooter({ onBack, onNext, nextLabel = "Continue →", nextDisabled = false, submitting = false }: {
  ...
  return (
    <div className="flex justify-between items-center px-6 py-4 border-t border-border bg-background shrink-0">
      ...
      <button type="button" onClick={onNext} disabled={nextDisabled || submitting}
...
// ─── Main Modal ───────────────────────────────────────────────────────────────
export default function CreateInitiativeModalDashboard({
  ...
  // Pre-fill submitter fields from profile
  useEffect(() => {
  ...
  const step2Valid =
    ...
    form.specificAsk.trim().length > 0
    ...
    >
      <div className="bg-background rounded-2xl border border-border w-full max-w-lg shadow-xl flex flex-col"
        ...
        <div className="flex items-start justify-between gap-4 px-6 pt-5 pb-4 border-b border-border shrink-0">
          <div className="flex-1"><StepBar current={step} /></div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            ...
            {step === 0 && (
              <div className="space-y-6">
                <StepHeader step={1} title="The basics" />
                <div>
                ...
                <SectorSelector selected={form.sectors} onChange={(sectors) => setForm(f => ({ ...f, sectors }))} />
                <div>
                ...
                <div>
                  ...
                  <div className="flex justify-end mt-1">
                    <p className={`text-xs ${wordCount(form.targetPopulation) > 30 ? 'text-red-500 font-semibold' : 'text-muted-foreground'}`}>
                      {wordCount(form.targetPopulation)}/30 words
                    </p>
                  </div>
                  <HintText>Be specific — this helps partners assess whether your constituency matches theirs.</HintText>
                </div>
              </div>
            )}

            {/* Step 1: Problem, Outcome, Stage, Budget */}
            {step === 1 && (
              <div className="space-y-6">
                <StepHeader step={2} title="Challenge & outcome" />
                <div>
                  <FieldLabel required>Problem statement</FieldLabel>
                  <textarea placeholder="What specific problem does this initiative address? (max 30 words)"
                    value={form.problem}
                ...
                <div>
                  ...
                  <textarea placeholder="What measurable outcome will this initiative achieve? (max 30 words)"
                    value={form.outcome}
                ...
                <div>
                  <FieldLabel required>Where is this initiative right now?</FieldLabel>
                ...
                <div>
                      ...
                      <button key={opt.value} type="button"
                        ...
                        className={cn(
                          ...
                          form.confirmedAssets.includes(opt.value)
                            ? "border-primary bg-[#fdf5f2] text-primary"
                  ...
                  <HintText>Select all that apply. Helps partners assess momentum.</HintText>
                </div>
                <div>
            ...
            {step === 2 && (
              <div className="space-y-7">
                <StepHeader step={3} title="Partnership needs" />
                <div>
                ...
                <div>
                  <FieldLabel required>Describe the specific ask</FieldLabel>
                  <textarea
                    placeholder="What would a partner actually do or contribute? Be direct — e.g. 'Fund a 6-month pilot at $40,000' or 'Provide an M&E specialist for 10 hrs/month'."
                    value={form.specificAsk}
                    onChange={e => set("specificAsk", e.target.value)}
                    rows={3}
                    className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                  />
                  <div className="flex justify-end mt-1">
                    <p className={`text-xs ${wordCount(form.specificAsk) > 80 ? 'text-red-500 font-semibold' : 'text-muted-foreground'}`}>
                      {wordCount(form.specificAsk)}/80 words
                    </p>
                  </div>
                  <HintText>Vague asks reduce response quality. Say exactly what you need.</HintText>
                </div>
            ...
            {/* Step 3: Prior experience (new) */}
            {step === 3 && (
              <div className="space-y-6">
                <StepHeader step={4} title="Your experience" />
                <div>
                  ...
                  <div className="space-y-2 mt-1">
                    {[
                      ...
                      <button key={String(opt.value)} type="button"
                        onClick={() => set("hadPriorExperience", opt.value)}
                        className={cn(
                          "w-full text-left px-5 py-3.5 rounded-xl border text-sm font-semibold transition-colors",
                          form.hadPriorExperience === opt.value
                            ? "border-primary bg-[#fdf5f2] text-primary"
                            : "border-border hover:border-foreground/20 text-foreground"
                        )}>
                        {opt.label}
                ...
                {form.hadPriorExperience === true && (
                  <div>
                    ...
                    />
                    <div className="flex justify-end mt-1">
                      <p className={`text-xs ${wordCount(form.priorExperienceDetail) > 60 ? 'text-red-500 font-semibold' : 'text-muted-foreground'}`}>
                        {wordCount(form.priorExperienceDetail)}/60 words
                      </p>
                    </div>
                    <HintText>Name it, state the scale, and share one measurable result. This is the single detail most likely to convert interest into engagement.</HintText>
                  </div>
                )}
              </div>
            )}

            {/* Step 4: Full detail (optional) */}
            {step === 4 && (
              <div className="space-y-6">
                <StepHeader step={5} title="Initiative detail" />
                <div>
                ...
                <div>
                  ...
                  <HintText>Link to a pitch deck, concept note, or proposal</HintText>
                </div>
              </div>
            )}
            ...
            {step === 5 && (
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-border bg-muted/30 px-4 divide-y divide-border">
                  ...
                  <ReviewRow label="Serves"            value={form.targetPopulation} />
                  <ReviewRow label="Stage"             value={STAGE_OPTIONS.find(s => s.value === form.stage)?.label ?? "—"} />
                  <ReviewRow label="Confirmed"
                    value={form.confirmedAssets.length > 0
                      ? form.confirmedAssets.map(a => CONFIRMED_ASSET_OPTIONS.find(o => o.value === a)?.label ?? a).join(", ")
                      : "—"} />
                  ...
                  <ReviewRow label="Partnerships"
                    value={form.partnerships
                      .map((p) => PARTNERSHIP_OPTIONS.find((o) => o.value === p)?.label ?? p)
                      .join(", ")} />
                  <ReviewRow label="Specific ask"      value={form.specificAsk} />
                  <ReviewRow label="Start date"        value={form.startDate || "—"} />
                  <ReviewRow label="Duration"          value={form.duration  || "—"} />
                  <ReviewRow label="ESG alignment"
                    value={form.esg === true ? "Yes" : form.esg === false ? "No" : "—"} />
                  <ReviewRow label="Prior experience"
                    value={form.hadPriorExperience === true ? "Yes" : form.hadPriorExperience === false ? "No" : "—"} />
                  {form.priorExperienceDetail && (
                    <ReviewRow label="Prior detail"    value={form.priorExperienceDetail} />
                  )}
                ...
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {form.submitterOrg && (
                      <p className="text-xs text-muted-foreground">{form.submitterOrg}</p>
                    )}
                    <p className="text-xs text-muted-foreground">{form.submitterEmail}</p>
                  </div>
                  <p className="text-xs text-muted-foreground mt-3 leading-relaxed">
                    To update your name or email, edit your{" "}
        ...
        {/* Success — no sign-in CTA since user is already logged in */}
        {submitted && (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8 py-12">
            ...
            <p className="text-sm text-muted-foreground max-w-xs leading-relaxed">
              Your initiative has been received and is pending review. It will appear in the marketplace once approved.
              We'll reach out via email with next steps.
            </p>
            <div className="mt-6 flex flex-col items-center gap-3">
              <button type="button" onClick={() => { setStep(0); setForm(INITIAL_STATE); setSubmitted(false); setError(null) }}
        ...
        {!submitted && step === 3 && (
          <StepFooter onBack={() => setStep(2)} onNext={() => setStep(4)} nextDisabled={!step3Valid} />
        )}
        {!submitted && step === 4 && (
          <StepFooter onBack={() => setStep(3)} onNext={() => setStep(5)}
            nextDisabled={!!(form.resourceLink && !urlValid(form.resourceLink))}
          />
        )}
        {!submitted && step === 5 && (
          <StepFooter onBack={() => setStep(4)} onNext={handleSubmit}

...
function TierCard({ tier }: { tier: TierDef }) {
  ...
  return (
    <div
      className={`rounded-2xl border flex flex-col px-7 ${isElevated ? "py-8" : "py-7"} ${cardStyle}`}
    >
      {tier.badge && (
        <div
          className={`inline-flex items-center gap-1 self-start mb-3 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${accentChip} ${isDark ? "text-white" : "text-foreground"}`}
        >
          {tier.badge}
        </div>
      )}
      ...
      <p className={`text-sm mb-4 leading-snug ${isDark ? "text-white/60" : "text-muted-foreground"}`}>
        {tier.blurb}
      </p>
      <div className="mb-6">
        <p className={`font-extrabold tracking-tight ${isElevated ? "text-4xl" : "text-3xl"} ${isDark ? "text-white" : "text-foreground"}`}>
          ...
          <span className={`text-sm font-medium ml-1 ${isDark ? "text-white/50" : "text-muted-foreground"}`}>/mo</span>
        </p>
        {tier.priceNgn > 0 && (
      ...
      <div className="flex flex-col gap-2.5 mb-8 flex-1">
        {tier.features.map((f) => (
          <div key={f} className="flex items-start gap-2.5">
            <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${accentChip}`}>
...
export default function PricingPage() {
  return (
    <div className="w-full">
      {/* ── HERO ── */}
      <section
        className="relative w-full min-h-[45vh] flex items-center overflow-hidden"
        style={{
          width: "100vw",
      ...
      >
        ...
        <div className="relative z-10 max-w-screen-2xl mx-auto content-padding hp-hero py-20 md:py-24 w-full">
          ...
          <p className="text-lg md:text-xl text-white/70 leading-relaxed max-w-2xl">
            Start free, upgrade when you need AI-assisted matching, evaluation, or audit-ready compliance tools.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-screen-2xl mx-auto content-padding hp-hero py-16 md:py-24">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {TIERS.map((tier) => (
            <TierCard key={tier.value} tier={tier} />
          ))}
        </div>

...
//
// Fix 3: the matched org's display card (name aside) was built entirely
// from organizations.partnership_sought / partnership_stage /
// partnership_budget -- the legacy, org-wide mirrored fields that only
// ever reflect whichever listing was saved most recently. Confirmed live:
// an org's card showed "Seeking Technology partnership for accelerator"
// from a draft listing, while the org's actual published (and correctly
// matched) listing was about something else entirely. The backend now
// returns matched_listing_id on every match (refresh-partnership-matches
// v22+); this fetches that specific listing's own sought/stage/budget/
// needs instead. organisation_name/organisation_type/country still come
// from organizations -- those are genuinely org-level, shared across all
// of an org's listings, not per-ask.
